...
col_pal = pals::alphabet(n = 22)

p1 = ggplot() +
  geom_segment(data = ddata$segments, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_point(data = ddata$labels, aes(x = x, y = leaf_height, color = state, shape = collection_period), size = 3) +
  scale_color_manual(values = unname(col_pal)) +
...
pdf("figures/pop_gen/phylogeny/Nf_ML_tree_polar.pdf", height = 10, width = 12)
p2
dev.off()
...
p3 = ggplot() + 
  ...
  scale_fill_manual(values =  unname(col_pal)) +
...
pdf("figures/pop_gen/phylogeny/Nf_ML_tree.k_4.pdf", height = 12, width = 12)
p3
dev.off()
