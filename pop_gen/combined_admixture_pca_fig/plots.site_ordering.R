...
pdf("figures/pop_gen/pca_and_admixture.site_order.pdf", width = 25.5, height = 6.35)
...
p9 = ggplot(Nd.strK_5.clust, 
    ...
    theme(
        ...
        strip.placement = "outside",
        axis.title.x = element_blank(),
        plot.title = element_text(hjust = -0.055, vjust = -0.25),
        panel.spacing = unit(0.0,'lines')
    )
p9

p9.tree = ggplot(Nd.strK_5.clust, 
                aes(
                    x = reorder(Sequence_label, MDS1), 
                    y = log(Q*100+1), 
                    #y = Q,
...
gp4 = ggplotGrob(p4)
gp5 = ggplotGrob(p5)
gp6 = ggplotGrob(p6.new)
gp7 = ggplotGrob(p7.new)
gp8 = ggplotGrob(p8)
gp9 = ggplotGrob(p9)
...
gpTop = cbind(gp2, gp1, gp5, gp3, gp4)
gpMiddle = cbind(gp6, gp7)
gpBottom = cbind(gp8, gp9)

ncol(gpTop)
ncol(gpMiddle)
gpMiddle.add = gtable_add_cols(gpMiddle, widths = rep(unit(1, "points"),5), pos = 1)
ncol(gpMiddle.add)
gpMiddle.add = gtable_add_cols(gpMiddle.add, widths = rep(unit(1, "points"),10), pos = -1)
...
pdf("figures/pop_gen/pca_admixture_structure.site_order.pdf", width = 25.5, height = 8.9)
grid.arrange(gpTop, gpBottom.add, gpMiddle.add, heights = c(0.43,0.25,0.31))
dev.off()
